...
public class FontReplacer : EditorWindow
{
    ...
    private static void ReplaceFonts(Font src, Font dest, bool includePrefabs)
    {
        ...
        //var scenes = sceneGuids.Select(guid => UnityEngine.SceneManagement.SceneManager.GetSceneByPath(AssetDatabase.GUIDToAssetPath(guid)));
        for(int i = 0; i < sceneGuids.Length; i++)
        {
            ...
            {
                matches += ReplaceFonts(src, dest, go.GetComponentsInChildren<Text>(true));
        ...
        if (includePrefabs)
        {
            ...
            {
                int matches = ReplaceFonts(src, dest, prefab.GetComponentsInChildren<Text>(true));
