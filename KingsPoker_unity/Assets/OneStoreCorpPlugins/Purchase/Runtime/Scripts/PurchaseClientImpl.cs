...
namespace OneStore.Purchasing
{
    public class PurchaseClientImpl : IPurchaseExtensions
    {
        ...
        private volatile Dictionary<
            ProductType,
            AsyncRequestStatus
        > _queryProductDetailsCallStatus = new Dictionary<ProductType, AsyncRequestStatus>
        {
            { ProductType.INAPP, AsyncRequestStatus.Succeed },
            { ProductType.SUBS, AsyncRequestStatus.Succeed },
            { ProductType.AUTO, AsyncRequestStatus.Succeed },
        ...
        private void StartConnection(Action action)
        {
            ...
            purchaseClientStateListener.OnSetupFinished += (javaIapResult) =>
            {
                ...
                {
                    _logger.Error(
                        "Failed to connect to service with error code '{0}' and message: '{1}'.",
                        iapResult.Code,
                        iapResult.Message
                    );
        ...
        {
            _productInPurchaseFlow = null;
            if (!IsPurchasingServiceAvailable())
                return;
        ...
        public void QueryProductDetails(ReadOnlyCollection<string> productIds, ProductType type)
        {
            ...
            ExecuteService(() =>
            {
                ...
                var productDetailsParamsBuilder = new AndroidJavaObject(
                    Constants.ProductDetailsParamBuilder
                );
                productDetailsParamsBuilder.Call<AndroidJavaObject>(
                    Constants.ProductDetailsParamBuilderSetProductIdListMethod,
                    JniHelper.CreateJavaArrayList(productIds.ToArray())
                );
                productDetailsParamsBuilder.Call<AndroidJavaObject>(
                    Constants.ProductDetailParamBuilderSetProductTypeMethod,
                    type.ToString()
                );

                var productDetailsParams = productDetailsParamsBuilder.Call<AndroidJavaObject>(
                    Constants.BuildMethod
                );
                ...
                RunOnMainThread(
                    () =>
                        _callback.OnPurchaseFailed(
                            new IapResult((int)ResponseCode.ERROR_ILLEGAL_ARGUMENT, message)
                        )
                );
                return;
            }
        ...
        public void Purchase(PurchaseFlowParams purchaseFlowParams)
        {
            if (_productInPurchaseFlow != null)
            {
                var message = string.Format(
                    "A purchase for {0} is already in progress.",
                    _productInPurchaseFlow
                );
                _logger.Error(message);
                RunOnMainThread(
                    () =>
                        _callback.OnPurchaseFailed(
                            new IapResult((int)ResponseCode.RESULT_ERROR, message)
                        )
                );
                return;
            }
            ...
            ExecuteService(() =>
            {
                ...
                using (
                    var purchaseFlowParamsBuilder = new AndroidJavaObject(
                        Constants.PurchaseFlowParamsBuilder
                    )
                )
                {
                    purchaseFlowParamsBuilder.Call<AndroidJavaObject>(
                        Constants.PurchaseFlowParamsBuilderSetProductIdMethod,
                        purchaseFlowParams.ProductId
                    );
                    purchaseFlowParamsBuilder.Call<AndroidJavaObject>(
                        Constants.PurchaseFlowParamsBuilderSetProductTypeMethod,
                        purchaseFlowParams.ProductType
                    );
                    purchaseFlowParamsBuilder.Call<AndroidJavaObject>(
                        Constants.PurchaseFlowParamsBuilderSetQuantityMethod,
                        purchaseFlowParams.Quantity
                    );
                    purchaseFlowParamsBuilder.Call<AndroidJavaObject>(
                        Constants.PurchaseFlowParamsBuilderSetDeveloperPayloadMethod,
                        purchaseFlowParams.DeveloperPayload
                    );

                    purchaseFlowParamsBuilder.Call<AndroidJavaObject>(
                        Constants.PurchaseFlowParamsBuilderSetProductNameMethod,
                        purchaseFlowParams.ProductName
                    );

                    purchaseFlowParamsBuilder.Call<AndroidJavaObject>(
                        Constants.PurchaseFlowParamsBuilderSetGameUserIdMethod,
                        purchaseFlowParams.GameUserId
                    );
                    purchaseFlowParamsBuilder.Call<AndroidJavaObject>(
                        Constants.PurchaseFlowParamsBuilderSetPromotionApplicableMethod,
                        purchaseFlowParams.PromotinApplicable
                    );
        ...
        public void UpdateSubscription(PurchaseFlowParams purchaseFlowParams)
        {
            ...
            {
                var message = string.Format(
                    "The update subscription for {0} is already in progress.",
                    _productInPurchaseFlow
                );
                _logger.Error(message);
                RunOnMainThread(
            ...
            ExecuteService(() =>
            {
                ...
                )
                {
                    ...
                    using (
                        var updateParamsBuilder = new AndroidJavaObject(
                            Constants.SubscriptionUpdateParamsBuilder
                        )
                    )
                    {
                        updateParamsBuilder.Call<AndroidJavaObject>(
                            Constants.SubscriptionUpdateParamsBuilderSetProrationModeMehtod,
                            purchaseFlowParams.ProrationMode
                        );
                        updateParamsBuilder.Call<AndroidJavaObject>(
                            Constants.SubscriptionUpdateParamsBuilderSetOldPurchaseTokenMethod,
                            purchaseFlowParams.OldPurchaseToken
                        );
            ...
            ExecuteService(() =>
            {
                ...
                )
                {
                    consumeParamsBuilder.Call<AndroidJavaObject>(
                        Constants.ConsumeParamsBuilderSetPurchaseDataMethod,
                        purchaseData.ToJava()
                    );
        ...
        public void AcknowledgePurchase(PurchaseData purchaseData, ProductType type)
        {
            ...
            {
                using (
                    var acknowledgeParamsBuilder = new AndroidJavaObject(
                        Constants.AcknowledgeParamsBuilder
                    )
                )
                {
                    acknowledgeParamsBuilder.Call<AndroidJavaObject>(
                        Constants.AcknowledgeParamsBuilderSetPurchaseDataMethod,
                        purchaseData.ToJava()
                    );
        ...
        public void ManageRecurringProduct(PurchaseData purchaseData, RecurringAction action)
        {
            ...
            {
                using (
                    var recurringParamsBuilder = new AndroidJavaObject(
                        Constants.RecurringProductParamsBuilder
                    )
                )
                {
                    recurringParamsBuilder.Call<AndroidJavaObject>(
                        Constants.RecurringProductParamsBuilderSetPurchaseDataMethod,
                        purchaseData.ToJava()
                    );
                    recurringParamsBuilder.Call<AndroidJavaObject>(
                        Constants.RecurringProductParamsBuilderSetRecurringActionMethod,
                        action.ToString()
                    );

                    var recurringProductListener = new RecurringProductListener(
        ...
        {
            using (
                var subscriptionParamsBuilder = new AndroidJavaObject(
                    Constants.SubscriptionParamsBuilder
                )
            )
            {
                if (purchaseData != null)
                {
                    subscriptionParamsBuilder.Call<AndroidJavaObject>(
                        Constants.SubscriptionParamsBuilderSetPurchaseDataMethod,
                        purchaseData.ToJava()
                    );
                }
        ...
        private void GetStoreCode()
        {
            ...
            _purchaseClient.Call(Constants.PurchaseClientGetStoreInfoMethod, storeCodeListener);
        }
        ...
        private bool IsPurchasingServiceAvailable()
        {
            ...
            var message = string.Format(
                "Purchasing service unavailable. ConnectionStatus: {0}",
        ...
        private void ProcessProductDetailsResult(
            ProductType type,
            AndroidJavaObject javaIapResult,
            AndroidJavaObject javaProductDetailList
        )
        {
            var iapResult = _iapHelper.ParseJavaIapResult(javaIapResult);
            if (!iapResult.IsSuccessful())
            {
                _logger.Warning(
                    "Retrieve product failed with error code '{0}' and message: '{1}'",
                    iapResult.Code,
                    iapResult.Message
                );
            ...
            var productDetailList = _iapHelper.ParseProductDetailsResult(
                javaIapResult,
        ...
        private void ProcessPurchaseUpdatedResult(
            AndroidJavaObject javaIapResult,
            AndroidJavaObject javaPurchasesList
        )
        {
            _productInPurchaseFlow = null;
            var iapResult = _iapHelper.ParseJavaIapResult(javaIapResult);
            ...
            {
                _logger.Warning(
                    "Purchase failed with error code '{0}' and message: '{1}'",
                    iapResult.Code,
                    iapResult.Message
                );
        ...
        private void ProcessQueryPurchasesResult(
            ProductType type,
            AndroidJavaObject javaIapResult,
            AndroidJavaObject javaPurchasesList
        )
        ...
        private void ProcessConsumePurchaseResult(
            AndroidJavaObject javaIapResult,
            AndroidJavaObject javaPurchaseData
        )
        {
            var iapResult = _iapHelper.ParseJavaIapResult(javaIapResult);
            if (!iapResult.IsSuccessful())
            {
                _logger.Error(
                    "Failed to finish the consume purchase with error code {0} and message: {1}",
                    iapResult.Code,
                    iapResult.Message
                );
        ...
        )
        {
            ...
            {
                _logger.Error(
                    "Failed to finish the acknowledge purchase with error code {0} and message: {1}",
                    iapResult.Code,
                    iapResult.Message
                );
        ...
        private void ProcessRecurringProductResult(
            AndroidJavaObject javaIapResult,
            string productId,
        ...
        )
        {
            ...
            {
                _logger.Error(
                    "Failed to finish the manage recurring with error code {0} and message: {1}",
                    iapResult.Code,
                    iapResult.Message
                );
                ...
                HandleErrorCode(
                    ...
                    () =>
                    {
                        RunOnMainThread(
                            () =>
                                _callback.OnManageRecurringProduct(
                                    iapResult,
                                    null,
                                    RecurringAction.Get(recurringAction)
                                )
                        );
                    }
                );
        ...
        private void QueryPurchasesInternal(
            string productId,
            ProductType type,
            Action<IapResult, PurchaseData> callback
        )
        {
            var queryPurchasesListener = new QueryPurchasesListener(type);
            queryPurchasesListener.OnPurchasesResponse += (_, javaIapResult, javaPurchasesList) =>
            {
                ...
                {
                    _logger.Warning(
                        "QueryPurchasesInternal failed with error code '{0}' and message: '{1}'",
                        iapResult.Code,
                        iapResult.Message
                    );
